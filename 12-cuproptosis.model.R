...
setwd("C:\\Users\\86188\\Desktop\\LUAD-Cu\\12.model")      #设置工作目录
...
############对数据进行分组#############
  ...
  # # print(sort(pbc.obj$importance,decreasing = TRUE)[1:10])
  # vs.pbc <- var.select(object = pbc.obj)
  # topvars <- vs.pbc$topvars
  # topvars
  # uniSigExp <- uniSigExp[,topvars]
  # uniSigExp <- cbind(tcga.exp[,1:2],uniSigExp)
  ...
  if((pValue<0.01) & (roc$AUC[2]>0.71) & (pValueTest<0.02) & (rocTest$AUC[2]>0.7)){
    #输出分组结果
