...
setwd("C:\\Users\\86188\\Desktop\\LUAD-Cu\\09.z-ggalluvial")     #设置工作目录

#读取输入文件
rt=read.table(inputFile, header=T, sep="\t", check.names=F)
#设置颜色
mycol=rainbow(length(unique(rt[,1])), s=0.8, v=0.8)
...
#绘制桑基图
p1<-ggplot(data=rt, aes(axis1 =lncRNA , axis2 =immuneGene, y = 1))+
  geom_alluvium(aes(fill =immuneGene), width = 0.1, knot.pos = 0.1, reverse = F)+ 
